...
import type { UpdatePasswordInput, UpdateProfileInput, UpdateSettingsInput, UserRole } from '@app/shared';
...
export async function listUsers(params: ListUsersParams): Promise<PaginatedResult<ReturnType<typeof toUserDto>>> {
  ...
  if (params.q) filter.name = { $regex: escapeRegex(params.q), $options: 'i' };
